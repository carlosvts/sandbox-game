...
{
    GuiPanel({GUIPANEL_X, GUIPANEL_Y, GUIPANEL_WIDTH, GUIPANEL_HEIGHT},
             "Particles");
    if (GuiButton({BUTTONPANEL_X, BUTTONPANEL_Y, BUTTON_WIDTH, BUTTON_HEIGHT},
                  "SAND"))
    {
        return SAND;
    }
    if (GuiButton(
            {BUTTONPANEL_X, BUTTONPANEL_Y + 35, BUTTON_WIDTH, BUTTON_HEIGHT},
            "WATER"))
    {
        return WATER;
    }
    if (GuiButton({BUTTONPANEL_X, BUTTONPANEL_Y + 35 * 2, BUTTON_WIDTH,
                   BUTTON_HEIGHT},
                  "FIRE"))
    {
        return FIRE;
    }
    if (GuiButton({BUTTONPANEL_X, BUTTONPANEL_Y + 35 * 3, BUTTON_WIDTH,
                   BUTTON_HEIGHT},
                  "IRON"))
    {
        return IRON;
    }
    if (GuiButton({BUTTONPANEL_X, BUTTONPANEL_Y + 35 * 4, BUTTON_WIDTH,
                   BUTTON_HEIGHT},
                  "GAS"))
    {
        return GAS;
    }
    if (GuiButton({BUTTONPANEL_X, BUTTONPANEL_Y + 35 * 5, BUTTON_WIDTH,
                   BUTTON_HEIGHT},
                  "LAVA"))
    {
        return LAVA;
    }
    if (GuiButton({BUTTONPANEL_X, BUTTONPANEL_Y + 35 * 6, BUTTON_WIDTH,
                   BUTTON_HEIGHT},
                  "CONWAY"))
    {
        return CONWAY;
    }
    if (GuiButton({BUTTONPANEL_X, BUTTONPANEL_Y + 35 * 7, BUTTON_WIDTH,
                   BUTTON_HEIGHT},
                  "VOID"))
    {
        return VOID;
...
void saveParticles(std::vector<Particle> &grid, int selectedElement)
{
    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) &&
        !CheckCollisionPointRec(
            GetMousePosition(),
            {GUIPANEL_X, GUIPANEL_Y, GUIPANEL_WIDTH, GUIPANEL_HEIGHT}))
    {
        int posX = GetMouseX() / SCALE;
        int posY = GetMouseY() / SCALE;
        if (posX >= GRID_WIDTH || posY >= GRID_HEIGHT || posX < 0 || posY < 0)
        {
            return;
        }
        ...
        particle.rect = {static_cast<float>(posX * SCALE),
                         static_cast<float>(posY * SCALE), SCALE, SCALE};
...
void calculatePhysics(std::vector<Particle> &grid)
{
    ...
    for (int i = grid.size() - 1; i >= 0; --i)
    {
        ...
        if (self->type == SAND)
        {
            ...
            if (index > static_cast<int>(grid.size() - 1))
            {
                continue;
            }
            Particle *other = &grid.at(index);
            if ((other->type == VOID || other->type == WATER) &&
                !other->wasUpdated)
            {
        ...
        else if (self->type == WATER)
        {
            ...
            int leftIndex = i - 1;
            int rightIndex = i + 1;
            if (leftIndex <= 0 || rightIndex >= static_cast<int>(grid.size()) ||
                i <= 0 || i >= static_cast<int>(grid.size()))
            {
                continue;
            }
            // std::vector<Particle*> others = {&grid.at(leftIndex),
            // &grid.at(index), &grid.at(rightIndex)};
            ...
            if (index < static_cast<int>(grid.size()))
            {
                ...
                if ((under->type == VOID || !under->exists) &&
                    !under->wasUpdated)
                {
                ...
                else if (under->type == IRON || under->type == WATER ||
                         under->type == SAND || under->type == GAS)
                {
                    ...
                    for (int attempt = 0; attempt < 2; attempt++)
                    {
                        ...
                        else if (randomness == 1 && canGoRight &&
                                 rightIndex <=
        ...
        else if (self->type == FIRE)
        {
            ...
            for (int y = -1; y <= 1; ++y)
            {
                for (int x = -1; x <= 1; ++x)
                {
                    ...
                    int g_height = grid.size() / GRID_WIDTH;
                    if (neighborX >= 0 && neighborX < GRID_WIDTH &&
                        neighborY >= 0 && neighborY < g_height)
                    {
                        ...
                        {
                            // if hits a gas or conway, convolutes all Moore
                            // neighbor to fire
                            for (int ky = -1; ky <= 1; ++ky)
                            {
                                for (int kx = -1; kx <= 1; ++kx)
                                {
                                    ...
                                    {
                                        continue;
                                    } // just a randomness to more

                                    int k_neighborX = (i % GRID_WIDTH) + kx;
                                    int k_neighborY = (i / GRID_WIDTH) + ky;
                                    if (k_neighborX >= 0 &&
                                        k_neighborX < GRID_WIDTH &&
                                        k_neighborY >= 0 &&
                                        k_neighborY < g_height)
                                    {
                                        int k_index = k_neighborY * GRID_WIDTH +
                                                      k_neighborX;
                                        Particle *k_particle =
                                            &grid.at(k_index);
        ...
        else if (self->type == GAS)
        {
            ...
            if ((above->type == VOID || above->type == WATER) &&
                !above->wasUpdated)
            {
            ...
            for (int attempts = 0; attempts < 2; attempts++)
            {
                ...
                else if (randomness == 0 && canGoRight &&
                         rightIndex < static_cast<int>(grid.size()) - 1)
                {
                    Particle *right = &grid.at(rightIndex);
                    if (right->type == VOID)
                    {
        ...
        else if (self->type == LAVA)
        {
            ...
            if (index < static_cast<int>(grid.size()))
            {
                ...
                // if the under is void, just go down again
                if ((under->type == VOID || !under->exists ||
                     under->type == WATER || under->type == IRON ||
                     under->type == GAS || under->type == FIRE) &&
                    !under->wasUpdated)
                {
                    under->type = self->type;
                    under->color = self->color;
                ...
                else if (under->type == LAVA)
                {
                    ...
                    // adds extra sauce with conway in a really unique
                    // circunstance
                    for (int y = -1; y <= 1; y++)
                    {
                        for (int x = -1; x <= 1; x++)
                        {
                            ...
                            if (checkX >= 0 && checkX < GRID_WIDTH &&
                                checkY >= 0 &&
                                checkY < (int)(grid.size() / GRID_WIDTH))
                            {
                                int targetIdx = checkY * GRID_WIDTH + checkX;
                                Particle *neighbor = &grid[targetIdx];

                                // if lava encounters water, it can generate a
                                // conway particle
                                if (neighbor->type == WATER)
                                {
                                    // 20%  (CONWAY), 80% (GAS)
        ...
        else if (self->type == CONWAY)
        {
            ...
            for (int y = -1; y <= 1; y++)
            {
                for (int x = -1; x <= 1; x++)
                {
                    ...
                    if (checkX >= 0 && checkX < GRID_WIDTH && checkY >= 0 &&
                        checkY < (int)(grid.size() / GRID_WIDTH))
                    {
                        int targetIdx = checkY * GRID_WIDTH + checkX;
                        if (grid.at(targetIdx).exists)
                        {
                            neighbors++;
                        }
                    }
            ...
            else
            {
                ...
                if (GetRandomValue(0, 100) <=
                    30) // 30% chance of infecting other particle
                {
                    ...
                    for (int x = 0; x < 4; x++)
                    {
                        ...
                        if (targetIdx >= 0 &&
                            targetIdx < static_cast<int>(grid.size()))
                        {
                            bool isSide = (offsets[x] == -1 || offsets[x] == 1);
                            if (isSide)
                            {
                                if ((offsets[x] == -1 && i % GRID_WIDTH == 0) ||
                                    (offsets[x] == 1 &&
