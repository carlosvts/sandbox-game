...
struct BMPInfoHeader {
  ...
  uint32_t colors_important{0}; // Num of colors used for displaying the bitmap.
...
struct BMP {
  ...
  // Constructors
  BMP(const char *bmpFile) { read(bmpFile); }
  ...
  // We will read a bmp, create another instance, modify it and write in another
    ...
    if (!istream.is_open()) {
      ...
      if (infoHeader.bitCount == bitsInBGR) {
        ...
        for (int i = 0; i < infoHeader.height; ++i) {
          istream.read(
              reinterpret_cast<char *>(&data[i * infoHeader.width * bitsInBGR]),
              infoHeader.width * bitsInBGR);
          // jumps padding
        ...
        for (int i = 0; i < infoHeader.height; ++i) {
          istream.read(reinterpret_cast<char *>(
                           &data[i * infoHeader.width * bitsInBGRA]),
                       infoHeader.width * bitsInBGRA);
        }
      }
    }
  }
  ...
  void toSepia() {
    ...
    for (size_t i = 0; i < data.size(); i += channels) {
      ...
      uint8_t newBlue = std::clamp(
          static_cast<int>((red * 0.272) + (green * 0.534) + (blue * 0.131)), 0,
          255);
      uint8_t newGreen = std::clamp(
          static_cast<int>((red * 0.394) + (green * 0.686) + (blue * 0.168)), 0,
          255);
      uint8_t newRed = std::clamp(
          static_cast<int>((red * 0.393) + (green * 0.769) + (blue * 0.189)), 0,
          255);
        ...
        for (int c = 0; c < 3; ++c) {
          ...
          for (int ky = -1; ky <= 1; ++ky) {
            ...
            for (int kx = -1; kx <= 1; ++kx) {
              int neighborIndex =
                  ((i + ky) * infoHeader.width + (j + kx)) * channels + c;
  ...
  void sobelEdgeDetector() {
    ...
    for (int i = 1; i < infoHeader.height - 1; ++i) {
      for (int j = 1; j < infoHeader.width - 1; ++j) {
        ...
        int computedGradient =
            std::sqrt(std::pow(accumulatorX, 2) + std::pow(accumulatorY, 2));
        uint8_t edge =
